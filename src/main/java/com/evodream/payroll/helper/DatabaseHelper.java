...
public class DatabaseHelper {
	...
	public Integer updateBlob(byte[] data, String table, String idKaryawan) {
		Integer id = -1;
		
		PreparedStatement statement;
		try {
			statement = (PreparedStatement) connection.prepareStatement("UPDATE " + table + " SET fingerprint = ? WHERE nik = ?");
			statement.setBytes(1, data);
			statement.setString(2, idKaryawan);
	...
	public ResultSet join(String tableA, String tableB, String columnA, String columnB, String type) {
		...
		querySelect += " FROM " + tableA + " " + type + " JOIN " + tableB + " ON " + tableA + "." + columnA + " = "
				+ tableB + "." + columnB + " ";
		Statement stm = null;
		ResultSet resultSet = null;
		try {
	...
	private void where(Map<String, String> data, String type, String operator) {
		...
		while (iterator.hasNext()) {
			...
			if (!where) {
				...
				queryWhere += " " + type + " " + mapEntry.getKey().replaceAll("'", "\\\\'") + " " + operator + " '"
						+ mapEntry.getValue().replaceAll("'", "\\\\'") + "'";
			}
	...
	private void set(Map<String, String> data) {
		...
		while (iterator.hasNext()) {
			...
			if (!set) {
				if (mapEntry.getValue() == null) {
					querySet += mapEntry.getKey().replaceAll("'", "\\\\'") + " = NULL";
				} else {
